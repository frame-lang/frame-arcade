...
# once per session, on the rising edge of game_over.
...
func _post_score_if_needed() -> void:
    var state: String = fsm.get_state()
    if state == "game_over" and _last_top_state != "game_over":
        Arcade.record_score("breakout", fsm.get_score())
    _last_top_state = state
