...
| `CCA3` | 2026-05-13 | V1.3 Phase 0.9 follow-on. Dwarf and Pirate FSMs each gained `pick_step: int = 0` — a third deterministic-PRNG counter independent of `step` (attack rolls) and `attack_step` (axe throws). Drives the canon-section-3 destination picker that moved from `driver._pick_dwarf_destination()` into per-FSM `pick_destination(candidates, forbidden)` operations. Per-dwarf RNG state replaces the driver's shared `_dwarf_walk_rng: RandomNumberGenerator` (which used an un-seeded RNG that didn't actually replay deterministically across saves despite the comments claiming it did). Saves under `CCA2` reset on next launch. |
